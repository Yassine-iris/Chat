...
if (isset($_POST['pseudoi']) && isset($_POST['mdpi']) && isset($_POST['mdpic']) && $_POST['pseudoi'] !== "" && $_POST['mdpi'] !== "" && $_POST['mdpic'] !== "") // On regarde si les champs ont été remplis
{
    ...
    try
{
    $connect = new PDO('mysql:host=localhost;dbname=chat;charset=utf8', 'root', ''); // On se connecte a la base de donnée via MySQL
}

catch(Exception $e)
{
        die('Il y a une erreur...'.$e->getMessage()); // Si erreur l'afficher
}
$hashedpass = password_hash($_POST['mdpi'], PASSWORD_DEFAULT); // On chache le mot de passe pour des raisons de sécurité
...
<meta http-equiv="refresh" content="2; URL=login.php">'; // Afficher qu'on a bien été inscrit et le V vert puis on va rediriger l'utilisateur vers le site principal
?>
<?php
	}
	else
...
<meta http-equiv="refresh" content="5; URL=inscription.php"></div>'; // Sinon afficher que les mots de passes ne correspondent pas et le X rouge et rediriger vers le formulaire d'inscription
	}
}
...
<meta http-equiv="refresh" content="2; URL=inscription.php">
	<?php
}
?>